...
import {request, http} from '../../request/index'
...
Page({
  ...
  onShow: function () {
    ...
    const that=this
    const data = wx.getStorageSync('products')
    data.map((item) => {
      if (item.base_info_id == id) {
        that.setData({
          evaluateList: item.evaluate
        })
      }
    })
  ...
  async getGoodsDetail(id) {
    await WXAPI.goodsDetail(id)
      .then((res) => app.handleDestruction(res))
      .then((data) => {
        this.GoodsInfo = data
        let collect = wx.getStorageSync('collect') || []
        let isCollect = collect.some(
          (v) => v.basicInfo.id === this.GoodsInfo.basicInfo.id
        )
        this.setData({
          goodsObj: {
  ...
  productViewsRecord(obj) {
    ...
    let index = products.findIndex(
      (v) => v.pics[0].goodsId === obj.pics[0].goodsId
    )
